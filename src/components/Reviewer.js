...
const Reviewer = ({ user }) => {
    ...
    const addReviewerNote = async (task) => {
        const noteData = {
            task_id: task.id,
            reviewer_note: reviewerNote
        };
        try {
            await axios.post('http://localhost:5000/add_reviewer_note', noteData);
            fetchTasksForDate(filterDate);
    ...
    return (
        <div className="container reviewer-dashboard">
                        ...
                        <div key={task.id} className="task-box" onClick={() => handleTaskSelection(task)}>
                            ...
                            <div className="form-group">
                                ...
                                <input
                                    ...
                                    value={selectedTask && selectedTask.id === task.id ? reviewerNote : task.reviewer_note || ''}
                                    onChange={(e) => {
                            ...
                            {selectedTask && selectedTask.id === task.id && (
                                <button className="button" onClick={() => addReviewerNote(task)}>Save Note</button>
                            )}
                        </div>
                    ))}
